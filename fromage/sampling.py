...
def generate_data(size,palette,produit,Vfr=0.31,proba_rupture=0.5,dt=30):
    ...
    for nb_it in range(size):
        ...
        if rupture:
            T,T_air,liste_stages=utils.constructT_air_avec_rupture_chaine(chaine=objects.chaine(circuit=circuit))
        else:
            T,T_air,liste_stage=utils.constructT_air_sans_rupture_chaine(chaine=objects.chaine(circuit=circuit))
